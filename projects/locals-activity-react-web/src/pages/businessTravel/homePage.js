...
export default class HomePage extends React.Component {
	constructor (props) {
		...
		this.state = {
			...
			services: [
				{icon: `${OSS_PATH}/icon0.png`, name: '订房折扣'},
				{icon: `${OSS_PATH}/icon1.png`, name: '专属管家'},
				{icon: `${OSS_PATH}/icon2.png`, name: '现场接待'},
				{icon: `${OSS_PATH}/icon3.png`, name: '延迟退房'}
			],
			introductions: [
	...
	receive = async (e) => {
		...
		utils.gioTrack('business_travel_act_receive_btn')
	...
	download = (e) => {
		e.stopPropagation()
		utils.gioTrack('business_travel_act_download_app_btn')
	 ...
	 */
	validate () {
		const {mobile, username, verificationCode} = this.state
		if (!/^\d{11}$/.test(mobile)) {
	...
	handleClickMiniCode = (e) => {
		e.stopPropagation()
		utils.gioTrack('business_travel_act_mini_code_btn')
	}

	render () {
		const {banners, services, isSend, time, loading, errorVisible, errorMsg, introductions, tipVisible} = this.state
		return (
			<div className="home-container">
				...
				<div className="home-body">
					<div className="discount-title-row">
						<span className="title">路客贵宾尊享</span>
					...
					<div className="user-form">
						...
						<input type="text" placeholder="请输入您的公司名称（选填）" className="input-field"
				...
				}
				{
					...
					<div className="mask" onClick={this.closeTipDialog}>
						<div className="tip-dialog">
							<div className="main-tip">您已是金卡会员</div>
							<div className="second-tip">赶紧去预订吧</div>
							<div className="download-btn" onClick={this.download}></div>
						</div>
					</div>
				}
				{
