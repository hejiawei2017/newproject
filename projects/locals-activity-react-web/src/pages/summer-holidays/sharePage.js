...
class App extends React.Component {
	constructor (props) {
		...
		this.state = {
			...
			errorVisible: false,
			resultVisible: false,
			hadSetOtherActivity: false,
			errorMsgHadSetOtherActivity: '',
			leaderboard: []
		};
		this.timer = null;
	}
	...
	componentWillMount () {
		...
		if (params.r) {
			const reEntry = params.r;

			that.setState({reEntry});
		}
		if (params.d) {
			const curIndex = params.d;
			that.setState({curIndex});
		}
		if (params.i) {
			const shareInfo = params.i;
	...
	initTime = () => {
		let endTime
		const now = moment()
		const startTime = moment().set({hour: 21, minute: 0, second: 0, millisecond: 0})
		const endTimeOfDay = moment().set({hour: 24, minute: 0, second: 0, millisecond: 0})
		if (now.isAfter(startTime) && now.isBefore(endTimeOfDay)) {
			endTime = moment().set({hour: 21, minute: 0, second: 0, millisecond: 0}).add(1, 'day').unix()
			this.coverTime(endTime);
			return
		}
		endTime = startTime.unix()
		this.coverTime(endTime);
	}
	getAllInfo = (user_id) => {  //获取房源信息
		...
		axios.get(`https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com/localhomeqy/summer-holidays/dataHouse.json`, {
			params: {
	...
	setActivity = (user_id) => {  //发起活动
		...
		axios.post(`${Host}/report/index`, {
			...
			.then(function (response) {
				// token无效去登陆
				if (response.data.errorCode === INVALID_TOKEN) {
					adapter.signIn();
					return
				}
				if (response.data.success) {
					...
					that.setState({
						errorMsgHadSetOtherActivity: response.data.errorMsg,
						hadSetOtherActivity: true
					})
				}
			})
			.catch(function (err) {
	...
	getPhone (token) {
		...
		axios
			...
			.then(function (response) {
				...
				// 有token且有效
				if (response.data.data && response.data.data.platformUser) {
	...
	makePoster = () => {  //生成海报
		...
		if (this.state.hadSetOtherActivity) {
			this.setState({
				errorVisible: true,
				errorMsg: this.state.errorMsgHadSetOtherActivity
			})
			return
		}
		...
		adapter.skipToPoster({
			title: `帮我助力，抢0元美宿`,
			path: `${pages}`,
			imageUrl: `https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com/localhomeqy/summer-holidays/share-bg.jpg`
		});
	}
	formatTime = (num) => {
		if (num < 10) {
	...
	entryShare = (item) => {
		...
		const now = moment()
		const startTime = moment({hour: 21, minute: 0, second: 0, millisecond: 0})
		const endTime = moment({hour: 21, minute: 1, second: 0, millisecond: 0})
	...
	renderModal = () => {
		...
		modalDom = (
			<div className="model resultSuc">
				<div>
					<div className="model_text" onClick={(e) => {
						this.entryOffer(e)
					}}>
						<img
							src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/%E6%8C%89%E9%92%AE%20%E6%8B%B7%E8%B4%9D.png"
							alt=""/>
					</div>
					<img className="bg"
					     src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com/localhomeqy/boost/%E4%BB%8A%E6%97%A5%E5%B7%B2%E6%8A%A2%E5%AE%8C.png"
					     alt=""/>
				</div>
				<img
					src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/%E5%85%B3%E9%97%AD.png"
					className="backBtn" onClick={this.closeResultModel.bind(this)} alt=''/>
			</div>
		)
				...
				{sceneShare ?
					...
					<div className="sceneShare">
						<div className="pageBox">
							<img className="headBoxImage"
							     src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com/localhomeqy/summer-holidays/home-page-bg.png"
							     alt=''/>
						</div>
						<div className="share_display_section">
							<div className="share_dec_section">
								<div className="head_img">
									<img className="headBoxImage"
									     src={curUserInfo.avatar || "https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/tmphead.png"}
									     alt=''/>
								</div>
								<div className="status_title">
									<div
										className="status_title_info">{curUserInfo.nick_name ? curUserInfo.nick_name : ''}在抢:
									</div>
									{/*<div className="house_title">{curUserInfo.address ? curUserInfo.address : ''}0元住</div>*/}
									<div className="house_title">全国民宿通兑券</div>
						...
						<div className="homePage">
							<div className="panic_section">
								<div className="panicTitle">
									<p className="weightTitle"><span className="line"></span>·<span
										className="mainTitle">好友助力0元住，超赞美宿任你选</span>·<span className="line"></span></p>
									{/*<p className="subTitle">暑假期间 每晚10:00准点开抢</p>*/}
								</div>
								<div className="renderImage">
									{
										houseListData.map((item, index) => {
											return (
												<div key={index} className={`headBox ${item.open ? '' : 'house-mask'}`}
												     onClick={function () {
													     self.entryShare(item)
												     }}>
					...
					<div className="normal">
						<div className="shareBox">
							...
							<button className="show0riginBtn" onClick={() => {
								this.jumpPage(this.state.curData.ID)
							}}>查看房源
							</button>
						</div>
						<div className="share_display_section">
							<div className="share_dec_section">
								<div className="head_img">
									<img className="headBoxImage" src={this.state.avatar ? this.state.avatar : ''}
									     alt=''/>
								</div>
								<div className="status_title">
									<img
										src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/%E6%AF%8F%E5%A4%A9%E9%82%80%E8%AF%B7%E4%BA%BA%E6%95%B0%E6%9C%80%E5%A4%9A%20%E5%8D%B3%E5%8F%AF0%E5%85%83%E4%BD%8F.png"
										alt=""/>
									<p className="countTime"><span className="countTitle">剩余时间 </span> <span
										className="countTimeCard">{h + ':' + m + ':' + s}</span></p>
								</div>
							</div>
							<div className="share_btn_section">
								<button onClick={this.makePoster}>
									<img
										src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/%E9%82%80%E8%AF%B7%E5%A5%BD%E5%8F%8B%E5%8A%A9%E5%8A%9B.png"
										alt=""/>
								</button>
							</div>
							<div className="friend_section">
								<img className="friend_title"
								     src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/%E5%8A%A9%E5%8A%9B%E5%A5%BD%E5%8F%8B.png"
								     alt=""/>
								<div className="renderImage">
									{
									...
									<div className="headBox">
										<img className="headBoxImage"
										     src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com/localhomeqy/summer-holidays/plus.png"
										     alt=''/>
							...
							<div className="rank_section">
								<img className="rank_title"
								     src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/%E5%8A%A9%E5%8A%9B%E6%8E%92%E8%A1%8C%E6%A6%9C.png"
								     alt=""/>
								{leaderboard && leaderboard.length > 0 && <p className="rank_text">我的排名 <span
									style={{"color": "rgb(234, 122, 109)"}}>{this.state.myOrder ? `第${this.state.myOrder}名` : ''}</span>
								</p>}
							</div>
						</div>
											...
											<div className="ranking_header" key={index}>
												<div className="table_name">
													<div className="avator">
														{index === 0 ?
															<img className="winner"
															     src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/%E7%9A%87%E5%86%A0.png"
															     alt=""/>
															: null
														}
												...
												<div className="table_item">{index === 0 ? <span
													style={{"color": "#f00"}}>第{index + 1}名</span> : `第${index + 1}名`}</div>
											</div>
										)
									})
								}
				...
				}
				{
					...
					<div className='ruleTips' onClick={this.closeErrorTip.bind(this)}>
						<div className="model error">
							<img className="bg"
							     src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/WechatIMG88.png"
							     alt=""/>
							...
							<img
								src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/%E5%85%B3%E9%97%AD.png"
								className="backBtn" onClick={this.closeErrorTip.bind(this)} alt=''/>
						</div>
					</div>
				}
				{
					...
					<div className='ruleTips' onClick={this.closeSucTip.bind(this)}>
						<div className="model resultSuc">
							<div className="resultSucBox">
								<img className="bg"
								     src="https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/boost51/%E5%8A%A9%E5%8A%9B%E6%88%90%E5%8A%9F%E5%BC%B9%E7%AA%97.png"
								     alt=""/>
								<div className="model_text" onClick={(e) => {
									this.entryOffer(e)
								}}>
