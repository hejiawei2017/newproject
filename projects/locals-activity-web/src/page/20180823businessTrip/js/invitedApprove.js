import '../css/invitedApprove.less'
...
$(()=>{
    let nickName=getParam('nickName')
    let invitationCode=getParam('invitationCode')
    console.log(nickName,invitationCode)
    $('#invited-name').text(nickName)
  ...
  // 获取会员状态
  function getVipStatu(userId){
    $.ajax({
      type: 'GET',
      url: api + `/platform/cash-vip/user-id?userId=${userId}`,
      success: function (res) {
          if (res.success) {
              if ((res.data.isVip === 1)||(res.data.isVip === 0)) { //是会员要就提醒
                 $('.main-form').html(`您已经认证商务旅客，让${nickName}继续邀请其他人吧`)
                 $('.submit').css('display','none')
                 $('.go-home').css('display','block').click(()=>{location.href='./index.html'})
              } 
          } 
      },
      error: function (err) {
