...
*  @prop {Function} heightValueChange: 右边滑块回调 {heighValue：heighValue}
*/
...
Component({
  ...
  methods: {
    ...
    */
    ...
    isCanMove() {
      let { stepRpx, isCanOverlap, leftValue = 0, rightValue = 0 } = this.data
      if (!isCanOverlap) {
    ...
    setValueLeft() {
      this.setAutoValue('leftValue', this.data.lowValue, 'lowValueChange', 'lowValue')
    },
    setValueRight() {
      this.setAutoValue('rightValue', this.data.heightValue, 'heightValueChange', 'heightValue')
    },
    setAutoValue(valueName, targetValue, triggerEventName, triggerValueName) {
