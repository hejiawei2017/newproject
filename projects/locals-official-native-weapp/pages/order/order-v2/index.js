...
const { orderCalculate, createOrder, miniPay, getAvailableCoupons, payByCash, getV2BuyVipList, checkMemberCard } = require('../../../server/order')
...
Page({
  ...
  async init() {
    ...
    try {
      ...
      catchLoading(e)
      errorStatus = true
      if (e.errorMsg && e.errorMsg.indexOf('房源已被占用') !== -1) {
        setTimeout(() => {
          this.goDateSelectTap(false)
        }, 1000)
        return;
      }
      this.navigateBack()
  ...
  async calculateOrder() {
    ...
    try {
      ...
      // 是否使用储值金
      bookingRequest.useCash = isUseStoredMoney;
      bookingRequest.useCashMoney = bookingDetail.canUseCashMoneyForOrder;
  ...
  async setBuyVipData() {
    ...
    if (memberCardCode && !notRecommend.includes(memberCardCode)) {
      ...
      if (!this.buyMemberCardResponse) {
        const response = await getV2BuyVipList()
      ...
      vipList && vipList.forEach(item => {
        const { image, originalPrice } = memberCardInfo[item.memberCardCode]
        item.image = image
  ...
  selectBuyVip(e) {
    const { selectedBuyVipId, recommendBuy, buyMemberCardMap, memberCard } = this.data
    let { buyId, memberCardCode } = e.currentTarget.dataset
    // 如果当前用户已经是会员，就return掉点击事件
    if (memberCard.memberCardCode === 'SILVER' || memberCard.memberCardCode === 'GOLD'){
      return;
    }
    // 已经被选中，不再处理下面逻辑
    if (selectedBuyVipId === buyId) {
      return;
    }

    let data = {}
    if (memberCardCode !== recommendBuy.memberCardCode) {
      const newRecommendBuy = buyMemberCardMap.get(memberCardCode)
      if (!newRecommendBuy) return;
      getApp().mtj.trackEvent('order_select_member', {
        member_level: newRecommendBuy.memberCardName,
      });
      data.recommendBuy = newRecommendBuy
    }
    data.selectedBuyVipId = buyId;
    data.isBuyVip = true;
    this.setData(data, this.calculateOrder)
  },
  switchBuyVip() {
    const { recommendBuy } = this.data
    const isBuyVip = !this.data.isBuyVip
    if (isBuyVip) {
      getApp().mtj.trackEvent('order_toggle_buy_vip');
      gioTrack('order_toggle_buy_vip');
    }else {
      gioTrack('order_toggle_close_buy_vip');
    }
    this.setData({
      isBuyVip,
      selectedBuyVipId: isBuyVip && recommendBuy ? recommendBuy.buyId : ''
    }, this.calculateOrder)
  },
          ...
          .then(res => {
            let payParams = {
              ...
              fail: () => {
                ...
                wx.redirectTo({
                  url: '../book-pay-failed/index?bookingId=' + bookingId
                })
              }
            }
  ...
  goDateSelectTap(willFlash) {
    let url = '/pages/housing/date-select/date-select?houseId=' + this.data.houseSourceId
    if (willFlash) {
      url += '&willFlash=1'
    }
    wx.navigateTo({
  ...
  switchUseStoredMoney() {
    this.setData({
      isUseStoredMoney: !this.data.isUseStoredMoney
    }, this.calculateOrder)
  },
  selectDate() {
    wx.navigateTo({
  ...
  toUnsubscribe() {
    ...
    const { checkinStartDate } = this.data;
    wx.navigateTo({
      url: `/pages/tipspage/unsubscribe/index?searchStartDate=${checkinStartDate}`
    })
  },
  toRules() {
  ...
  showCouponTip() {
    wx.showModal({
      icon: 'none',
      title: '优惠券说明',
      content: `1、优惠券可直接抵扣路客任意房源房费\r\n2、自购买七天后可用，每三个月可使用一张`,
      showCancel: false,
      confirmText: '知道了'
    })
  },
      ...
      switch (key) {
        case 'isShowBuyVipList':
          if (result) {
            ...
            gioTrack('order_show_more_member');
          }
          break;
      }
      // 跳转到新页面查看费用明细
      if (key === 'isShowPriceDetail') {
        const {
          bookingRequest,
          accidentAmountTotal,
          selectCoupon,
        ...
        wx.setStorageSync('locals_mini_order_price_detail', {
          ...
          clearPrice: bookingRequest.clearPrice,
          deposit: bookingRequest.deposit,
          memberBagCode: memberCard.memberCardCode, // 当前会员Code
          buyMemberBagPrice: bookingRequest.memberBagPrice,
          buyMemberBagText: (recommendBuy && selectedBuyVipId === recommendBuy.buyId) ? `升级${recommendBuy.memberCardName}会员` : '',
          // 区分显示选择了推荐升级会员的折扣还是原始的当前会员折扣, 选择了推荐会员且折扣是有值的
          buyMemberBagCode: (recommendBuy && selectedBuyVipId === recommendBuy.buyId) ? recommendBuy.memberCardCode : null,
          accidentAmountTotal,
          couponName: selectCoupon && selectCoupon.coupon ? selectCoupon.coupon.couponName : '优惠券',
          couponAmount: bookingRequest.couponAmount,
          discountAmount: bookingRequest.discountAmount,
          useCash: bookingRequest.useCash,
          useCashMoney: bookingRequest.useCashMoney,
          payTotalPrice
        });
        wx.navigateTo({
          url: '/pages/order/order-price/index'
        })
        gioTrack('order_show_price_detail');
        return;
      }
      this.setData({
        [key]: result,
  ...
  // 检查是否显示会员升级模块
  async checkShowBugVip() {
    const userInfo = wx.getStorageSync('userInfo')
    const { data } = await checkMemberCard({userId:userInfo.id})
    this.setData({
      isShowBugVip: data && data.boughtMemberCard ? data.boughtMemberCard : false
    })
  }
})
