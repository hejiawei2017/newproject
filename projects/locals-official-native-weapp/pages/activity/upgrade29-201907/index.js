...
const app = getApp()
const { miniPay, waitPay } = require('../../../server/order')
...
const { addInviteRecord } = require('../../../server/marketing')
const { checkMemberCard } = require('../../../server/order')
...
Page({
  ...
  data: {
    ...
    pageImgs: {
      headerImg:
        'https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/up_grade_201907/up_grade_header.png',
      blackCard:
        'https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/black_card.png',
      goldCard:
        'https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/gold_card.png',
      silverCard:
        'https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/silver_card.png',
      guarantees:
        'https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/8guarantees.png',
      dialogTitle:
        'https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/up_grade_201907/dialog_title.png',
      blackCardSuccess:
        'https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/up_grade_201907/black_success.png',
      goldCardSuccess:
        'https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/up_grade_201907/up_grade_gold_susscess.png',
      silverCardSuccess:
        'https://locals-house-prod.oss-cn-shenzhen.aliyuncs.com//localhomeqy/up_grade_201907/silver_success.png'
    },
  ...
  onLoad: function(options) {
    ...
    if (options.channel) {
      ...
      gioTrack('upgrade_channel', { channel: options.channel })
    }
    if (options.sid) {
    ...
    if (options.openfn) {
      ...
      if (
        options.openfn !== 'black' &&
        options.openfn !== 'gold' &&
        options.openfn !== 'silver'
      ) {
        return
      }
  ...
  setCardBuyersCount(key, timeout) {
    ...
    let count =
      this.data.cardBuyersCount[key] || this.randomCount(400000, 300000)
  ...
  getDistanceTime() {
    ...
    endTime = new Date(
      isNaN(parseInt(endTime) ? endTime : parseInt(endTime))
    ).getTime()
    ...
    if (diff < -3 * 60 * 60 * 24 * 1000) {
      ...
      const startTime =
        endTime +
        parseInt(diff / (3 * 60 * 60 * 24 * 1000)) * (60 * 60 * 24 * 1000) * 3
      // 当前循环内的活动结束时间
  ...
  async signInCallback() {
    // 判断是否购买过会员卡
    const checkCardRes = await checkMemberCard({
      userId: app.globalData.userInfo.id
    })

    const { success, data } = checkCardRes
    ...
    const { boughtMemberCard = false } = data

    if (boughtMemberCard) {
      wx.showToast({
        icon: 'none',
        title: '您已购买过会员升级套餐，每人仅限购买1次'
      })
      return
    }
  ...
  radioChange(e) {
    ...
    if (
      myCardType &&
      cardTypes.indexOf(tapCardType) >= cardTypes.indexOf(myCardType)
    )
      return
  ...
  getRecommendedCardType(type) {
    ...
    if (!myCardType || cardTypes.indexOf(type) < cardTypes.indexOf(myCardType))
      return type
  ...
  gioTrack2CardTypeProxy() {
    ...
    const opt = channel ? { channel } : {} //上报添加渠道
    name && gioTrack(name, opt)
  },
  /**
  ...
  longpressShow() {
    ...
    if (inviteCode) {
      showText =
        parseInt(selfInviteCode) === parseInt(inviteCode)
          ? `邀请编号:${inviteCode}(本人)`
          : `邀请编号:${inviteCode}`
    } else {
      showText = '欢迎购买'
