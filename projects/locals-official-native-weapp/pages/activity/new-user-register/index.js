const request = require('../../../utils/request');
const { validator, shareDataFormat } = require('../../../utils/util');
// const BASE_API = 'http://127.0.0.1:7001/api/';
const BASE_API = 'https://i.localhome.cn/api/';
const { joinShare, statisticsEvent } = require('../../../server/hd');
// const BASE_API = 'http://tp.localhome.cn:9999/api/';
const DEFAULT_OVERTIME = 60;
const ACTIVITY_NAME = 'active_register';
const app = getApp();
let t = 0;

Page({
  data: {
    overTime: DEFAULT_OVERTIME,
    modalHidden: true,
    phone: '',
    code: '',
    channel:null
  },

  onLoad: function(options) {
    let { channel } = options;
    // 记录渠道号
    channel && this.setData({channel:channel})
    
    const userInfo = app.globalData.userInfo;
    console.log(userInfo.isNew)
    if (userInfo && userInfo.isNew === false) {
      wx.showModal({
        title: '温馨提示',
        content: '你已是路客用户了，赶紧邀请好友领更多优惠券吧~',
        showCancel: false,
        confirmText: '马上邀请',
        success(res) {
          wx.redirectTo({
            url: '/pages/activity/new-user-register/poster/index',
          });
        },
      });
    }
  },

  onShow() {
    // this.selectComponent("#auth-drawer-box").checkRole()
    wx.pageScrollTo({ scrollTop: 500 });
  },
  // 点击手机号码框授权登录
  signIn(){
    this.selectComponent("#auth-drawer-box").checkRole()
  },
  // 登录成功后回调
  signInCallback(){
    const channel = wx.getStorageSync('from_channel');
    if (channel) {
      joinShare({
        ticket_id: ACTIVITY_NAME,
        share_user_id: channel,
      });
    }

    request.post(`${BASE_API}new_use_redpacket/index`, {
      phone:app.globalData.userInfo.mobile,
      userInfo: JSON.stringify(app.globalData.userInfo),
      traceId: app.globalData.sid,
      activity_id: "1902220547571",
    })
    .then(response => {
      this.setData({ modalHidden: false });
    })
    .catch((e = {}) => {
      console.info('e', e)
      if(e.errorMsg.indexOf("已存在")){
        wx.showModal({
          title: '提示',
          showCancel: false,
          content: '您已经领取啦',
          success(res) {
            wx.redirectTo({
              url: '/pages/activity/new-user-register/poster/index',
            });
          },
        });
      }else{
        wx.showModal({
          title: '提示',
          showCancel: false,
          content: e.errorMsg || e.errorDetail || '服务器繁忙',
        });
      }
    });
  },
  /**
   * 重置定时器
   *
   */
  resetTiming() {
    clearInterval(t);
    this.setData({
      overTime: DEFAULT_OVERTIME,
    });
  },

  /**
   * 输入框事件
   *
   * @param {*} e
   */
  changeInput(e) {
    let key = e.currentTarget.dataset.name;
    let value = e.detail.value;
    this.setData({ [key]: value });
  },

  /**
   * 获取验证码失败
   *
   */
  getCodeFail() {
    wx.showModal({
      title: '提示',
      content: '获取验证码失败， 请稍后再试',
      showCancel: false,
    });
    this.resetTiming();
  },

  /**
   * 专挑到Vip页面
   *
   */
  jumpToVip() {
    wx.navigateTo({
      url: '/pages/activity/upgrade29-201812/index?channel=menucard',
    })
  },

  /**
   * 获取验证码
   *
   * @returns
   */
  getCode() {
    const { phone, overTime } = this.data;
    if (!validator(phone, 'phone')) {
      return;
    }
    if (overTime != DEFAULT_OVERTIME) {
      return;
    }
    this.setData({ overTime: overTime - 1 });
    t = setInterval(() => {
      let { overTime } = this.data;
      if (overTime <= 0) {
        this.resetTiming();
        return;
      }
      this.setData({ overTime: overTime - 1 });
    }, 999);

    request
      .post('/platform/auth/auth-code/send', {
        mobile: phone,
      })
      .then(e => {
        if (!e.success) {
          this.getCodeFail();
          return;
        }
        wx.showToast({
          icon: 'success',
          title: e.data,
        });
      })
      .catch(() => {
        this.getCodeFail();
      });
  },

  /**
   * 点击领取优惠券
   * 1. 绑定手机号
   * 2. 绑定成功则向手机号发送优惠券
   * 3. 向发起邀请用户发送优惠券
   * @returns
   */
  receiveCoupons() {
    let { phone, code } = this.data;
    if (!validator(phone, 'phone')) {
      return;
    }
    if (code.length <= 3) {
      wx.showToast({
        title: '验证码错误，请重新输入',
        icon: 'none',
      });
      return;
    }
    request
      .put('/platform/user/user-info-mobile', {
        mobile: phone,
        code,
        traceId: app.globalData.sid,
      })
      .catch(e => {
        return e.errorCode === '20105' ? e : true;
      })
      .then(response => {
        if (typeof response !== 'boolean' && !response.success) {
          throw response;
        }
        // 记录渠道
        getApp().mtj.trackEvent('active_register');
          
        app.getUserInfoDetail();
        // 向后台发送优惠券请求
        return request.post(`${BASE_API}new_use_redpacket/index`, {
          phone,
          userInfo: JSON.stringify(app.globalData.userInfo),
          traceId: app.globalData.sid,
          activity_id: "1902220547571",
        });
      })
      .then(response => {
        this.setData({ modalHidden: false });
      })
      .catch((e = {}) => {
        console.info('e', e)
        wx.showModal({
          title: '提示',
          showCancel: false,
          content: e.errorMsg || e.errorDetail || '服务器繁忙',
        });
      });
  },

  onUnload: function() {
    this.resetTiming();
  },

  onShareAppMessage: function() {
    // 用户点击右上角分享
    return shareDataFormat({
      title: '新人红包 100 元，首单立减', // 分享标题
      desc: '覆盖全球 65+ 城市，超 15000+ 套个性民宿', // 分享描述
      path: '/pages/activity/new-user-register/index', // 分享路径
    });
  },
});
