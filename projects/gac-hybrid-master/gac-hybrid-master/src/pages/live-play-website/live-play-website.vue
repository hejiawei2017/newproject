...
export default {
  ...
  created() {
    ...
    this.id = util.jsUrlHelper.getUrlParam(window.location.href, "id")||10863;
    // this.wxCode = util.jsUrlHelper.getUrlParam(window.location.href, "wxCode");

    //测试
    // this.id = 1320;
    // this.wxCode = "033oeDa90s9xvz1gmHc909Uda90oeDaO";
    // this.token = `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..AvIDCCtvgssIvGX-j08LIfHG_g8ULcMZoR9vJ1dPbQw`;
  },
  mounted() {
    let vm = this;
  ...
  methods: {
    ...
    loaddData() {
      if (!this.id) {
        this.$refs.showTips.showTips("缺少参数-id，请稍后再试");
        return;
      }
      return axios({
        method: "get",
